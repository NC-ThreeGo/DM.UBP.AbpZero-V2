...
namespace DM.UBP.Tests
{
    ...
    /// Provides methods to easily work with <see cref="UBPDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<UBPTestModule>
    {
        ...
        protected void UsingDbContext(Action<UBPDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<UBPDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<UBPDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<UBPDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<UBPDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<UBPDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<UBPDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<UBPDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<UBPDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<UBPDbContext, Task<T>> func)
        {
            T result;
